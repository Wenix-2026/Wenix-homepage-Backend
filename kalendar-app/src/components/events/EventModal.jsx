...
export function EventModal({ open, onClose, onSave, onDelete, initialDate, event }) {
  ...
  useEffect(() => {
    if (event) {
      setForm({
        ...
        assigneeIds:      event.event_assignees?.map((ea) => ea.profiles.id) || [],
      })
  ...
  async function handleSubmit(e) {
    ...
    setSaveError(null)
    try {
      // Neprůstřelné parsování data (zvládne strojový i čistě český formát)
      const parseDateSafe = (dateString) => {
        if (!dateString) return new Date();

        // Pokud input vrací český formát např. "28.06.2026 18:08"
        ...
        // Pokud je to už klasický strojový formát (fallback)
