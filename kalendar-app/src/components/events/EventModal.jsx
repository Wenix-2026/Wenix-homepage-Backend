...
// Avatar pro ručně napsaný e-mail mimo profiles.
...
function localInputToIso(localStr) {
    ...
    const get = (type) => Number(partsInTz.find((p) => p.type === type)?.value)
    const shownAsUtc = Date.UTC(get('year'), get('month') - 1, get('day'), get('hour'), get('minute'), get('second') || 0)
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            if (initialData?.start_time) {
                ...
                setStartStr(isoToLocalInput(base.toISOString()))
            }

            if (initialData?.end_time) {
                setEndStr(isoToLocalInput(initialData.end_time))
            } else {
                const base = selectedDate ? new Date(selectedDate) : new Date()
                base.setHours(10, 0, 0, 0)
                setEndStr(isoToLocalInput(base.toISOString()))
            }
        }
